...
class PermissionRoleSeeder extends Seeder
{
    protected $permissionStructure = [
        ...
        'Dashboard' => [
            'global' => [
                ['name' => 'view_dashboard', 'display_name' => 'Ver Dashboard'],
            ],
            'recinto' => [
                ['name' => 'view_dashboard', 'display_name' => 'Ver Dashboard'],
            ]
    ...
    private function createRoles(): void
    {
        foreach ($this->roles as $name => $roleData) {
            ...
            if ($roleData['permissions'] === 'ALL') {
                ...
                $foundPermissions = Permission::whereIn('name', $roleData['permissions'])->get();
                if ($foundPermissions->count() != count($roleData['permissions'])) {
                    $missing = array_diff(
                        $roleData['permissions'],
                        $foundPermissions->pluck('name')->toArray()
                    );
                }
                $role->permissions()->sync($permissionIds);
            }
    ...
    private function generateDescription($name, $group, $scope): string
    {
        $scopeText = [
        ...
        return "Permiso para {$name} en {$group} - {$scopeText[$scope]}";
    }
}
