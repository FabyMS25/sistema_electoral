        ...
        <div class="row align-items-center">
            ...
            <div class="col-md-2">
                <span class="text-muted">
                    ...
                    Votos: <span class="fw-bold" id="total-{{ $table->id }}">{{ $table->total_voters }}</span>
                </span>
            </div>
                ...
                <div class="d-flex gap-3 flex-wrap align-items-center">
                    @forelse($candidatesByCategory as $categoryCode => $candidates)
                        ...
                        <span class="badge bg-{{ $categoryColorMap[$categoryCode] ?? 'secondary' }} category-badge">
                            {{ $categoryCode }}:
                            <span id="total-{{ $categoryCode }}-{{ $table->id }}">{{ $categoryTotal }}</span>
                            @if(!$isConsistent)
    ...
    <div class="card-body p-0">
        ...
        <div class="row g-0 bg-light p-2 border-top small">
            <div class="col-md-3">
                ...
                <span class="fw-bold ms-1" id="footer-valid-{{ $table->id }}">
                    {{ array_sum(array_column($table->results_by_category ?? [], 'valid_votes')) }}
                </span>
            </div>
            <div class="col-md-3">
                <span class="text-muted">Votos en Blanco:</span>
                <span class="fw-bold ms-1" id="footer-blank-{{ $table->id }}">
                    {{ array_sum(array_column($table->results_by_category ?? [], 'blank_votes')) }}
                </span>
            </div>
            <div class="col-md-3">
                <span class="text-muted">Votos Nulos:</span>
                <span class="fw-bold ms-1" id="footer-null-{{ $table->id }}">
                    {{ array_sum(array_column($table->results_by_category ?? [], 'null_votes')) }}
                </span>
            </div>
            <div class="col-md-3">
                {{-- Papeletas sobrantes now comes from VotingTableElection --}}
                <span class="text-muted">No Utilizadas:</span>
                <span class="fw-bold ms-1">
                    {{ $table->elections->first()?->ballots_leftover ?? 0 }}
                </span>
            </div>
        </div>
                ...
                @foreach($table->results_by_category as $categoryCode => $result)
                    @if(!$result['is_consistent'])
                        <small class="text-warning d-block">
                            ...
                            (total guardado: {{ $result['total_votes'] }})
                        </small>
                    @endif
                @endforeach
            </div>
