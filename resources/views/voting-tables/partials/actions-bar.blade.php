...
<div class="row g-4 mb-3">
    ...
    <div class="col-sm">
        <div class="d-flex justify-content-sm-end">
            <form method="GET" action="{{ route('voting-tables.index') }}" class="d-flex">
                <div class="search-box me-2">
                    <input type="text" name="search" class="form-control" 
                           placeholder="Buscar mesa..." value="{{ request('search') }}">
                    <i class="ri-search-line search-icon"></i>
                </div>
                <button type="submit" class="btn btn-soft-primary">
                    <i class="ri-search-line"></i>
                </button>
                @if(request('search') || request('institution_id'))
                <a href="{{ route('voting-tables.index') }}" class="btn btn-soft-secondary ms-2">
